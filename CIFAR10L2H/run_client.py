...
if __name__ == "__main__":
    ...
    if args.start_at:
        now = int(time.time())
        sleep_for = args.start_at - now
        if sleep_for > 0:
            print(f"[clientV3] waiting {sleep_for}s for synchronized start...", flush=True)
...
'''
# clientV3.py (hardened: predictable outputs, robust logging, always saves artifacts)
import json
import os, sys, io, socket, struct, time, argparse, pickle
import numpy as np
import torch
...
parser.add_argument('--conn_timeout', type=float, default=30.0)
parser.add_argument('--retries', type=int, default=3)
parser.add_argument('--thresholds', type=str, default='0.0')  # comma-separated
args = parser.parse_args()
...
# ── Files / device ──────────────────────────────────────────────────────────
REJECTOR_PATH = 'rejector_70.pt'
CLIENT_MODEL_PATH = 'client_model70.pt'
device = "cuda" if torch.cuda.is_available() else "cpu"
print("[clientV3] device:", device)
    ...
    for attempt in range(1, retries + 1):
        try:
            with socket.create_connection((host, port), timeout=timeout) as sock:
                ...
                # Serialize CPU tensor
                buf = io.BytesIO()
                torch.save(batch_cpu, buf)
                batch_data = buf.getvalue()

                # Rejector logits as explicit little-endian float32
                rej_data = rejector_logits.detach().cpu().numpy().astype('<f4').tobytes()
...
if __name__ == "__main__":
    ...
    for threshold in thresholds:
        ...
        for _ in range(iterations):
            for imgs, labels in testloader:
                img_cpu = imgs  # keep on CPU for serialization
                img = imgs.to(device)
                label = labels.item()
                total += 1
                ...
                t_rej_start = time.perf_counter()
                with torch.inference_mode():
                    r_logits = rejector(img.view(1, -1))
                    prob = torch.softmax(r_logits, dim=1)[0, 1]
                t_rej_end = time.perf_counter()
                rej_time = max(0.0, min(0.02, t_rej_end - t_rej_start))
                stats["rej_time"].append(rej_time)
                ...
                if prob >= threshold:
                    ...
                    t0 = time.perf_counter()
                    logits, T2, T3, client_count, server_cps = request_batch_expert(
                        img_cpu, r_logits[0], host, port, args.conn_timeout, args.retries
                    )
                    t4 = time.perf_counter()

                    if logits is None:
                        # failed expert call; skip to next sample
                        continue

                    # Single-clock metrics
                    et  = max(0.0, T3 - T2)      # server compute (server clock)
                    rtt = max(0.0, t4 - t0)      # client RTT   (client clock)
                    net = max(0.0, rtt - et)     # rough net+queue
                    c2e = e2c = net / 2.0
        ...
        # NEW: overall average latency across all points (expert RTT + client compute)
        overall_avg = (sum_rtt + sum_ct) / total if total else 0.0
        stats.setdefault("overall_latency", []).append(overall_avg)

        print(f"[clientV3] Acc: {acc*100:.2f}% | Avg server RTT: {avg_lat:.4f}s | Overall avg: {avg_lat2:.4f}s", flush=True)
        time.sleep(1)
    ...
    if args.primary_client and args.num_clients:
        ...
        # ALWAYS save a histogram (partial data allowed)
        import matplotlib.pyplot as plt
        x_vals = [i + 1 for i in range(NUM_TESTS)]
        y_vals = [v if isinstance(v, (int, float)) and v > 0 else 0.0 for v in latency_list]
        plt.figure(figsize=(10, 5))
        ...
        plt.xlabel("Number of Clients")
        plt.ylabel("Latency (s)")
        plt.title("Number of Clients vs Total Latency")
        plt.tight_layout()
        out_path = os.path.join(BASE_DIR, "Client_vs_Latency_Histogram.png")
        plt.savefig(out_path)
        print(f"[clientV3] histogram saved: {out_path}", flush=True)

        # ── NEW 1: Overall latency (client+expert) per number of clients ─────────
        OVERALL_PICKLE = "overall_latency.pkl"
        if os.path.exists(OVERALL_PICKLE):
            with open(OVERALL_PICKLE, "rb") as f:
                overall_list = pickle.load(f)
        ...
        plt.xlabel("Number of Clients")
        plt.ylabel("Overall Avg Latency (s)")
        plt.title("Number of Clients vs Overall Average Latency (Expert RTT + Client Compute)")
        plt.tight_layout()
        out_overall = os.path.join(BASE_DIR, "Clients_vs_OverallLatency_Histogram.png")
        plt.savefig(out_overall)
        print(f"[clientV3] overall latency histogram saved: {out_overall}", flush=True)

        # ── NEW 2: Total expert calls per test (all clients) ─────────────────────
        CALLS_PICKLE = "expert_calls.pkl"
        if os.path.exists(CALLS_PICKLE):
            with open(CALLS_PICKLE, "rb") as f:
                calls_list = pickle.load(f)
        ...
        # ── NEW 3: Save average accuracy per threshold (no graph) ───────────────
        try:
            acc_by_threshold = {f"{thr:.6f}": float(a) for thr, a in zip(thresholds, stats["accs"])}
            with open("avg_accuracy.json", "w") as f:
                json.dump(acc_by_threshold, f, indent=2, sort_keys=True)
            print(f"[clientV3] saved avg_accuracy.json -> {os.path.abspath('avg_accuracy.json')}")
        except Exception as e:
            print(f"[clientV3] WARNING: failed to write avg_accuracy.json: {e}", flush=True)
