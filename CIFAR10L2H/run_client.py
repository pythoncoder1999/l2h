...
parser.add_argument('--thresholds', type=str, default='-0.079675')  # comma-separated
...
if __name__ == "__main__":
    ...
    thresholds = [-0.079675]#[float(x) for x in args.thresholds.split(',')] if args.thresholds else [0.0]
    iterations = 5
    client_cps = 0.0
    ...
    for threshold in thresholds:
        ...
        for _ in range(iterations):
            for imgs, labels in testloader:
                ...
                if prob2 > max(0.5+threshold, prob1):
                ...
                elif (prob1 > max(0.5+threshold, prob2)) or ((prob1 <= 0.5+threshold) and (prob2 <= 0.5+threshold))
                    # ── Client path ──
                    ...
                    raise Exception("Bad case reached")
        # Stats per threshold
